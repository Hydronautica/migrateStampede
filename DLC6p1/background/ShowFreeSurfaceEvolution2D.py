...
Amax=1.   #       % To set the scale of the z-axis plot
...
filename = 'EP_05155.bin'
#data = open(filename, "rb").read() # read all of file
#data = open(filename, "rb") # just open file
#Nx = struct.unpack("<i", data.read(1))[0]
data = open(filename,'rb')
dummy = np.fromfile(data,dtype=np.int32,count=1)[0]
Nx = np.fromfile(data,dtype=np.int32,count=1)[0]
print(Nx)
Ny = np.fromfile(data,dtype=np.int32,count=1)[0]
print(Ny)
dummy = np.fromfile(data,dtype=np.int32,count=1)[0]

dummy = np.fromfile(data,dtype=np.int32,count=1)[0]
X = np.fromfile(data,dtype=np.float64,count=Nx*Ny)
print(X)
Y = np.fromfile(data,dtype=np.float64,count=Nx*Ny)
print(Y)
dummy = np.fromfile(data,dtype=np.int32,count=1)[0]

dummy = np.fromfile(data,dtype=np.int32,count=1)[0]
E = np.fromfile(data,dtype=np.float64,count=Nx*Ny)
print(E)
